...
def parse_args(argv=None):
    ...
    # Added to choose image type
    parser.add_argument('--show', default=0, type=int,
                        help='Choose image type. RGB: 0, NonRigid: 1, Rigid: 2.')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    # Crear un elemento estructurante (kernel)
    kernel = np.ones((3, 3), np.uint8)  # Puedes ajustar el tamaño

    # Aplicar dilatación
    img_nonrigid_numpy = cv2.dilate(img_nonrigid_numpy, kernel, iterations=2)
    img_rigid_numpy = cv2.dilate(img_rigid_numpy, kernel, iterations=2)


    # ORB processing
    img_orb_numpy = (img_orb * 255).byte().cpu().numpy()

    # Adding ORB points to grayscale image
    img_orb_gray = cv2.cvtColor(img_orb_numpy, cv2.COLOR_BGR2GRAY)
    ...
    if len(kp) != 0:
        for k in kp:
            if img_nonrigid_numpy[int(k.pt[1]), int(k.pt[0])] != 255:
    ...
    if num_dets_to_consider == 0:
        return img_numpy, img_nonrigid_numpy, img_rigid_numpy, img_orb_gray
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy, img_nonrigid, img_rigid, img_orb = prep_display(preds, frame, None, None, undo_transform=False)
    
    if save_path is None:
        # Convert both images from BGR to RGB for matplotlib
        img_numpy = img_numpy[:, :, (2, 1, 0)]
        img_white = img_white[:, :, (2, 1, 0)]

        if(show == 0):
            cv2.imshow(path, img_numpy)
        elif(show == 1):
            cv2.imshow(path, img_nonrigid)
        elif(show == 2):
            cv2.imshow(path, img_rigid)
        elif(show == 3):
            cv2.imshow(path, img_orb)
        else:
            raise ValueError("Showing method incorrectly selected! Choose RGB: 0, NonRigid: 1, Rigid: 2.")
    else:
        # Save both images to disk
        cv2.imwrite(save_path.replace(".png", "_overlay.png"), img_numpy)
        cv2.imwrite(save_path.replace(".png", "_white.png"), img_white)
